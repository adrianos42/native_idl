...
pub(crate) mod layer;
pub(super) mod client_cargo;
pub(super) mod client_types;
pub(super) mod client_mod;

#[derive(Error, Debug, Clone)]
pub enum FFIClientError {
...
impl ToTokens for FFIClient {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.append_all(&self.module);
    }
}

impl fmt::Display for FFIClient {
                    ...
                    let (args, ret_ty) = match &field.ty {
                        TypeName::TypeFunction(value) => (
                            Some(match &value.args {
                                ...
                                _ => return Err(FFIClientError::UnexpectedType),
                            }),
                            &value.return_ty,
                        ),
