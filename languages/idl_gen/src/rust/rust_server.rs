...
impl ToTokens for RustServer {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.append_all(&self.module);
    }
}

impl fmt::Display for RustServer {
...
impl RustServer {
    pub fn generate(analyzer: &Analyzer) -> Result<Self, RustServerError> {
                    ...
                    let (args, ret_ty) = match &field.ty {
                        TypeName::TypeFunction(value) => (
                            Some(match &value.args {
                                ...
                                _ => return Err(RustServerError::UnexpectedType),
                            }),
                            &value.return_ty,
                        ),
...
impl RustImplMod {
    pub fn generate() -> Result<Self, ()> {
        let module = quote! {
            ...
            pub use idl_internal::{Uuid, StreamInstance, StreamReceiver, StreamSender};
        };
...
pub fn rust_impl_files(
    ...
    for analyzer in analyzers {
        ...
        let impl_mod = RustServer::generate(&analyzer).unwrap();
