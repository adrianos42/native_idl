...
use native_idl::{analyzer, module, parser};
            ...
            if let Ok(file_path) = root_uri.to_file_path() {
                if let Ok(mut dir) = fs::read_dir(file_path.as_path()).await {
                    ...
                    for (name, doc) in &state.documents {
                        if module_state.add_document(name).await.is_ok() {
                            module_state.update_parse(name, &doc.text).await;
                            module_state.update_analyze(name).await;
                        }
                    }
                }
            }
            ...
            if let Some(doc) = state.documents.get_mut(&file_name) {
                assert!(
                    language_id == "idl" && doc.doc_type == DocumentType::Idl
                        || language_id == "toml" && doc.doc_type == DocumentType::IdlSpec
                );

                let module_state = self.module_state.lock().await;
                let _ = module_state.add_document(&file_name).await;

                module_state.update_parse(&file_name, &text).await;
                if let Some(parser) = module_state.get_parser(&file_name).await {
                    self.update_diagnostics_parser(uri.clone(), parser).await;

                    module_state.update_analyze(&file_name).await;
                    if let Some(analyzer) = module_state.get_analyzer(&file_name).await {
