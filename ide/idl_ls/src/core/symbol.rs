        ...
        .filter_map(|node| {
            match node {
                ...
                parser::ParserNode::TypeList(value) => {
                    let name = value.ident.to_string();
                    let children = get_type_fields(&value.ty_list);
                    let detail = String::new();
                    let range = get_range_from_parser(value.range);
                    let selection_range = range; // TODO
                ...
                parser::ParserNode::Factory(value) => {
                    let name = value.ident.to_string();
                    let children = get_factory_fields(&value.fields);
                    let detail = String::new();
                    let range = get_range_from_parser(value.range);
                    let selection_range = range; // TODO
                ...
                parser::ParserNode::Stream(value) => {
                    let name = value.ident.to_string();
                    let children = get_stream_fields(&value.fields);
                    let detail = String::new();
                    let range = get_range_from_parser(value.range);
                    let selection_range = range; // TODO
...
fn get_stream_fields(value: &[parser::StructNode]) -> Vec<DocumentSymbol> {
    value
        .iter()
        .filter_map(|node| match node {
            parser::StructNode::StructField(value) => {
...
fn get_factory_fields(value: &[parser::InterfaceNode]) -> Vec<DocumentSymbol> {
    value
        .iter()
        .filter_map(|node| match node {
            parser::InterfaceNode::InterfaceField(value) => {
...
pub(crate) async fn get_document_symbol(
    ...
    let parser_s = module_state.get_parser(&file_name).await?;
