    ...
    if let Some(file_name) = file_name_from_uri(&uri) {
        ...
        if let Some(doc) = state.documents.get(&file_name) {
            ...
            if let Some(completions) =
                completion::Completion::try_complete(doc.text.as_str(), pos, &module_state)
            {
                let mut items = vec![];
                ...
                for node in &completions.nodes {
                    match node {
                        ...
                        completion::CompletionNode::KeywordName(name) => {
                            items.append(&mut get_keyword_name_completion(name));
                        }
...
fn get_keyword_name_completion(name: &str) -> Vec<CompletionItem> {
    vec![CompletionItem {
        kind: Some(CompletionItemKind::Keyword),
        label: name.to_owned(),
        ..CompletionItem::default()
